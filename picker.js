...
(function (window, document, Math) {
    ...
    function PickerScroll (el, options) {
...
// INSERT POINT: indicator._move
    ...
    Picker.prototype = {
        ...
        scrollInit: function (index, num) {
            ...
            //iscroll 初始化
            self.scrollArray[index] = new PickerScroll(id, {
                probeType: 3,
                scrollX: false,
                scrollY: true,
                useTransition: true,
            ...
            if ( num ) {
                self.scrollArray[index].scrollTo(0, -itemHeight * num, 0);
            }

            self.scrollSite(index, itemHeight);
        },

        //滚动距离计算
        scrollSite: function (index, itemHeight) {
            var self = this;
            var myScroll = self.scrollArray[index];
            ...
            //     $(".zx_select *").on("touchstart", self.touchStop);
            // });
            ...
            myScroll.on('scroll', function () {
                ...
                var wdHeight = document.body.scrollHeight - 1;

                if ( this.pointY < 1 ) {
                    var nowScroll = this.scroller.lastChild;//当前项节点
                    self.SetItemList(nowScroll, index);
                    self.isScrollTo = true;
                    this.scrollTo(0, this.maxScrollY, 0);
                } else if ( this.pointY > wdHeight ) {
                    var nowScroll = this.scroller.firstChild;//当前项节点
                    self.SetItemList(nowScroll, index);
                    self.isScrollTo = true;
                    this.scrollTo(0, 0, 0);
                }
            ...
            myScroll.on('scrollEnd', function () {
                // $(".zx_select *").off("touchstart", self.touchStop);
                if ( self.isScrollTo ) {
                ...
                nowEndY = this.y;
                var h = Math.abs(nowEndY % itemHeight);//滚动多余不足一行的高度
                var halfHeight = itemHeight / 2;//行一半的高度

                var moveY = nowEndY + h;
                if ( h >= halfHeight ) {
                    //超过行一半的高度，则滚动一行
                    moveY = nowEndY - itemHeight + h;
                }

                var num = parseInt(Math.abs(moveY) / itemHeight) + 2;//item index
                var nowScroll = this.scroller.childNodes[num];//当前项节点
                self.SetItemList(nowScroll, index);

                this.scrollTo(0, moveY, 0);
                // this.scrollTo(0, moveY, 300, IScroll.utils.ease.quadratic);
            });
        },
        ...
        showScrollList: function (dataList, index, isInit) {
            ...
            if( isInit ){
                ...
                if( num < self.arrayDepth ){
                    self.scrollArray[num].scrollTo(0, 0, 0);
                    setTimeout(function () {
